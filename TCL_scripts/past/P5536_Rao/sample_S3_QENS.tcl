# Sample Environment: CF-12 
# Sample:Li6PS5Br, 0.5mm gap, annular cell

# Doppler = 4.7 m/s, 75 mm
# set_chopper_config_with_roi DopHz ChopHz frame=1 time-bins 16<VdetChannels<51 68<VdetHeightBins<200 0<DopTbins<399
# set_chopper_config_with_roi 9.962 20.21 1 1000 16 50 0 255 0 999


# Direct neutrons & Analysed neutrons 3 time sets
# BKG mode; tAUX time channels [6.45 10.9 1 4.45 0 0] (set 0 analysed, set 1 direct, set 2 rest) (top to bottom)


# Chopper phase = -64 deg
# chom = 21.5 deg
# Chopper = BC = -2426 rpm
# background chopper = GC= 1213 rpm

histmem stop

histmem mode time
histmem preset 3600


drive tc1_temp0_setpoint 95 tc2_temp0_setpoint 100
wait 900 

# acquisition 10 blocks of 1 hour at 100K

samplename Li6PS5Br_100K
for {set idx0 0} {$idx0 < 10} {incr idx0} {
	broadcast loop $idx0 Li6PS5Br_100K
	broadcast temperature t02_Sensor00
	histmem start block
	newfile TOTAL_HISTOGRAM_XT TOTAL_HISTOGRAM_T RAW_TOTAL_HISTOGRAM_XTAUX RAW_TOTAL_HISTOGRAM_YTAUX
	save 
}

############################################################################
drive tc1_temp0_setpoint 195 tc2_temp0_setpoint 200
wait 900 

# acquisition 10 blocks of 1 hour at 200K

samplename Li6PS5Br_200K
for {set idx0 0} {$idx0 < 10} {incr idx0} {
	broadcast loop $idx0 Li6PS5Br_200K
	broadcast temperature t02_Sensor00
	histmem start block
	newfile TOTAL_HISTOGRAM_XT TOTAL_HISTOGRAM_T RAW_TOTAL_HISTOGRAM_XTAUX RAW_TOTAL_HISTOGRAM_YTAUX
	save 
}

#####################################################################################
drive tc1_temp0_setpoint 295 tc2_temp0_setpoint 300
wait 900 

# acquisition 10 blocks of 1 hour at 300K

samplename Li6PS5Br_300K
for {set idx0 0} {$idx0 < 10} {incr idx0} {
	broadcast loop $idx0 Li6PS5Br_300K
	broadcast temperature t02_Sensor00
	histmem start block
	newfile TOTAL_HISTOGRAM_XT TOTAL_HISTOGRAM_T RAW_TOTAL_HISTOGRAM_XTAUX RAW_TOTAL_HISTOGRAM_YTAUX
	save 
}

####################################################################################
drive tc1_temp0_setpoint 300 tc2_temp0_setpoint 400
wait 900 

# acquisition 10 blocks of 1 hour at 400K

samplename Li6PS5Br_400K
for {set idx0 0} {$idx0 < 10} {incr idx0} {
	broadcast loop $idx0 Li6PS5Br_400K
	broadcast temperature t02_Sensor00
	histmem start block
	newfile TOTAL_HISTOGRAM_XT TOTAL_HISTOGRAM_T RAW_TOTAL_HISTOGRAM_XTAUX RAW_TOTAL_HISTOGRAM_YTAUX
	save 
}

#################################################################################################
drive tc1_temp0_setpoint 300 tc2_temp0_setpoint 500
wait 900 

# acquisition 10 blocks of 1 hour at 500K

samplename Li6PS5Br_500K
for {set idx0 0} {$idx0 < 10} {incr idx0} {
	broadcast loop $idx0 Li6PS5Br_500K
	broadcast temperature t02_Sensor00
	histmem start block
	newfile TOTAL_HISTOGRAM_XT TOTAL_HISTOGRAM_T RAW_TOTAL_HISTOGRAM_XTAUX RAW_TOTAL_HISTOGRAM_YTAUX
	save 
}


#EFW while cooling from 500K to 300K

hset /doppler/ctrl/run_cmd 0
wait 300

set_chopper_config_with_roi 0 20.21 1 1000 16 50 0 255 0 999

hset /sample/tc1/temp0/setpoint 0K 

hset /sample/tc2/temp0/setpoint 300K
samplename S3_Li6PS5Br_300K
runscan dummy_motor 10 14 24 time 150 datatype { TOTAL_HISTOGRAM_XT TOTAL_HISTOGRAM_T }


#drive tc1_temp0_setpoint 295 tc2_temp0_setpoint 300


