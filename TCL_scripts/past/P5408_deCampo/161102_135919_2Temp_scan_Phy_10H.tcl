...
for {set idx0 0} {$idx0 < 12} {incr idx0} {
	...
	#run tc1_temp0_setpoint [expr $idx0*2+278]
	#drive tc2_temp0_setpoint [expr $idx0*2+278]
	#wait 240 
...
for {set idx0 0} {$idx0 < 27} {incr idx0} {
	broadcast target [expr $idx0*2+302]
	#drive tc2_temp0_setpoint [expr $idx0*2+302]
	#wait 240
	broadcast [tc1_temp0_setpoint] [tc2_temp0_setpoint]
	histmem start block
	save $idx0
