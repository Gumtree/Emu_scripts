...
# Start loop 3
# Measures 5K temp steps from 150 - 180K ie 7 temp points
# Data will be saved in one file as a list
# Counting time per measurement = 30 minutes
histmem mode time
histmem preset 1800
newfile TOTAL_HISTOGRAM_T
for {set idx1 0} {$idx1 < 7} {incr idx1} {
	if {$START_NUMBER <= $loopnumber} {
		broadcast Target temperature = [expr $idx1*5+150]
		drive tc2_temp6_setpoint [expr $idx1*5+150]
		wait 600
